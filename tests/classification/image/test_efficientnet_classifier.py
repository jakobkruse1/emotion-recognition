...
def test_efficientnet_workflow():
    ...
    shutil.rmtree(
        os.path.join("tests", "temp", "efficient"), ignore_errors=True
    )
    save_parameters = {"save_path": os.path.join("tests", "temp", "efficient")}
    classifier.save(save_parameters)
    assert os.path.exists(os.path.join("tests", "temp", "efficient"))
    assert os.path.exists(
        os.path.join("tests", "temp", "efficient", "saved_model.pb")
    )
    ...
    with pytest.raises(RuntimeError):
        new_classifier.save(
            {"save_path": os.path.join("tests", "temp", "efficient")}
        )
