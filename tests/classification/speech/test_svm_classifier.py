...
def test_svm_workflow():
    ...
    shutil.rmtree(os.path.join("tests", "temp", "svm"), ignore_errors=True)
    save_parameters = {"save_path": os.path.join("tests", "temp", "svm")}
    classifier.save(save_parameters)
    assert os.path.exists(os.path.join("tests", "temp", "svm", "model.pkl"))
    assert os.path.exists(os.path.join("tests", "temp", "svm", "scaler.pkl"))
    ...
    with pytest.raises(RuntimeError):
        new_classifier.save(
            {"save_path": os.path.join("tests", "temp", "svm")}
        )
