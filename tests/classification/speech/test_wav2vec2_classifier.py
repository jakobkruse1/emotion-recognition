...
def test_wav2vec2_workflow():
    ...
    shutil.rmtree(
        os.path.join("tests", "temp", "wav2vec2"), ignore_errors=True
    )
    save_parameters = {
        "save_path": os.path.join("tests", "temp", "wav2vec2", "wav2vec2.pth"),
        "extra_layer": 1024,
    }
    classifier.save(save_parameters)
    assert os.path.exists(
        os.path.join("tests", "temp", "wav2vec2", "wav2vec2.pth")
    )
        ...
        new_classifier.save(
            {
                "save_path": os.path.join(
                    "tests", "temp", "wav2vec2", "wav2vec2.pth"
                )
            }
        )
