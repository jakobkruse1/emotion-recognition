...
def test_resnet_workflow():
    ...
    shutil.rmtree("tests/temp/plant_mfcc_resnet", ignore_errors=True)
    save_parameters = {"save_path": "tests/temp/plant_mfcc_resnet"}
    classifier.save(save_parameters)
    assert os.path.exists("tests/temp/plant_mfcc_resnet/saved_model.pb")
    ...
    with pytest.raises(RuntimeError):
        new_classifier.save({"save_path": "tests/temp/plant_mfcc_resnet"})
